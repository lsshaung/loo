...
// Revision    : $Id: //dwh/DW_ocb/DW_axi_x2p/amba_dev/src/DW_axi_x2p_bcm21.v#15 $
// Author      : Doug Lee    2/20/05
// Description : DW_axi_x2p_bcm21.v Verilog module for DW_axi_x2p
//
// DesignWare IP ID: c1f75a9c
//
////////////////////////////////////////////////////////////////////////////////
...
reg                     init_dly_n;
  ...
  generate if ((VERIF_EN % 2) == 1) begin : GEN_HO_VE_ODD
    if ((F_SYNC_TYPE & 7) == 1) begin : GEN_HO_FST_EQ_1
      always @ (negedge clk_d or data_s or rst_d_n) begin : PROC_catch_last_data_VE_EVEN
        ...
        last_data_dyn <= ((clk_d_stopped==1'b1) ? data_s : data_s_delta_t) & {WIDTH{rst_d_n}} & {WIDTH{init_dly_n}};
  ...
  assign data_s_sel_0 = (clk_d_stopped==1'b1) ? data_s :
                        (VERIF_EN < 1)        ? data_s :
                                                ((data_s & ~data_select) | (last_data_dyn & data_select));

  assign data_s_sel_1 = (clk_d_stopped==1'b1) ? data_s :
                        (VERIF_EN < 2)        ? {WIDTH{1'b0}} :
                                                ((last_data_s_q & ~data_select) | (last_data_s_qq & data_select));
...
// SJ: This circuit is not just during normal operation and synchronization functionality is not required during its use.
// spyglass disable_block Ar_resetcross01
// SMD: There are paths between two sequential elements that are clocked by the same clock domain but have different asynchronous resets/sets
// SJ: When used in a reset synchronizer. Reset crossing is expected.
            test_hold        <= data_s;
...
`endif
  ...
  generate
    ...
    if (SVA_TYPE == 1) begin : GEN_SVATP_EQ_1
      ...
      DW_axi_x2p_sva01 #(WIDTH, (F_SYNC_TYPE & 7)) P_SYNC_HS (.*);
    end
